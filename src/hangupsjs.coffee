...
class HangupsJS extends Adapter
  ...
  connected: =>
    ...
    #@client.getselfinfo().then( (res) => @robot.logger.info res)
  ...
  membership_change: (msg) =>
    ...
    for participant in msg.membership_change.participant_ids
      Promise.join @getUser(participant.chat_id), (user) =>
        if msg.membership_change.type == "JOIN" then m = new EnterMessage(user, null, msg.event_id) #user contains .room
  ...
  getUser: (userId) =>
    @client.getentitybyid([userId]).then (res) => 
     for entity in res.entities
      ...
      {id: userId, first_name: senderProps.first_name, display_name: senderProps.display_name, 
      photo_url: senderProps.photo_url, emails: senderProps.emails }
  ...
  send: (envelope, strings...) ->
    ...
    segments = (builder.text msg for msg in strings)
    @client.sendchatmessage envelope.room, segments
